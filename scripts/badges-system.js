...
class BadgeSystem {
  ...
  initBadges() {
    return {
      ...
      jeune_ingenieur: {
        id: "jeune_ingenieur",
        name: "⚙️ Jeune ingénieur",
        description: "Réussir les 2 questions Technologie avancée en Sciences",
        category: "performance",
        xpBonus: 200,
        condition: (userData) => {
          // Tag badge-ingénieur posé sur Q9-Q10 de Sciences Technologie
          // Le flag est écrit dans localStorage par sciences_duolingo_section.html
          const flag = localStorage.getItem("curieux_badge_ingenieur");
          return flag === "true";
        },
      },
    };
        ...
        return {
          ...
          earnedDates: badges.reduce((acc, b) => {
            if (b.unlocked && b.unlockedAt)
              acc[b.id] = new Date(b.unlockedAt).getTime();
            return acc;
          }, {}),
          notifications: [],
        };
  ...
  saveUserBadges() {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(dashboardFormat),
    );
    ...
    console.log(
      "✅ Badges sauvegardés:",
      dashboardFormat.filter((b) => b.unlocked).length,
      "badges débloqués",
    );
  }
  ...
  getUserData() {
    ...
    if (officialXP) {
      try {
        ...
        console.log(
          "[BadgeSystem] Total XP recalculé:",
          userData.totalXP,
          "depuis sections:",
          bySection,
        );
    ...
    if (userData.totalXP === 0) {
      ...
      if (oldXP) {
        try {
          ...
          userData.totalXP = Object.values(xp).reduce(
  ...
  addBadgeXP(badgeId, xpAmount) {
    try {
      ...
      console.log(
        `✨ +${xpAmount} XP bonus badge ${badgeId} → total: ${xp.total}`,
      );
    } catch (e) {
  ...
  calculateProgress(badge, userData) {
    switch (badge.id) {
      ...
      case "all_subjects": {
        const completed = Object.values(userData.quizBySubject).filter(
          (v) => v > 0,
        ).length;
        return { current: completed, target: 6 };
      }
      case "jeune_ingenieur": {
        const flag = localStorage.getItem("curieux_badge_ingenieur");
        return { current: flag === "true" ? 1 : 0, target: 1 };
      }
      default:
  ...
  getStats() {
    ...
    const totalXPBonus = this.getEarnedBadges().reduce(
      (sum, badge) => sum + badge.xpBonus,
      0,
    );
